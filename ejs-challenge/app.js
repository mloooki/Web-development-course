...
app.get("/",function(req,res){
  ...
  Post.find({},function(err,foundedItems){
    if(err){
      console.log(err);
    } else{
      console.log("Get DATA Secceefuly");
    }
    if(foundedItems ===0){
      res.render("home",{homeParagraph:homeStartingContent,posts:posts});
    } else{

      res.render("home",{homeParagraph : homeStartingContent, posts: foundedItems});
    }
  })
  //res.render("home",{homeParagraph:homeStartingContent,posts:posts});
});
...
app.get("/posts/:postId",function(req,res){
  ...
  // var founded = "Not Found!";
  // for(var i = 0 ; i<posts.length;i++){
  //   if( _.lowerCase(posts[i].title) === requestedTitle){
  //     res.render("post",{title:posts[i].title,paragraph:posts[i].body});
  //     break;
  //   }else{
  ...
  Post.findOne({_id: requestedPostId}, function(err, post){
    if(err){
      ...
      res.render("post",{title:post.title,paragraph:post.body});
    }
